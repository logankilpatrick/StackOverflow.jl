...
Will return bountied questions given a tag. `pagesize` is the number of questions it will return,
...
"""
function getfeaturedquestions(page::Int = 1, pagesize::Int = 1, fromdate::String = "", todate::String = "",
                                order::String = "desc", sort::String = "activity", tags::String = "Julia",
                                site::String = "stackoverflow")
    tags = replace(tags, " " => "%20")

    if fromdate != "" && todate != ""
        r = HTTP.request("GET", "https://api.stackexchange.com/2.2/questions/featured?page=$page&pagesize=$pagesize&fromdate=$fromdate&todate=$todate&order=$order&sort=$sort&tagged=$tags&site=$site")
    else
        r = HTTP.request("GET", "https://api.stackexchange.com/2.2/questions/featured?page=$page&pagesize=$pagesize&order=$order&sort=$sort&tagged=$tags&site=$site")
    end
